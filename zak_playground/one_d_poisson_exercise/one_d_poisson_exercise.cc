...
// Step function namespace
namespace TanhSolnOneDPoisson {

    int alpha = 1;

    // Exact step function
    void get_exact_u(const Vector<double>& x, Vector<double>& u){
        u[0] = tanh(alpha*(x[0] - 0.5));
    }

    // Source required to make step function an exact solution
    void source_function(const Vector<double>& x, double& source) {
        const double u = alpha * (x[0] - 0.5);
        const double t = std::tanh(u);
        const double sech2 = 1.0 - t * t;

        source = -2.0 * alpha * alpha * t * sech2;
    }
...
void OneDPoissonProblem<ELEMENT>::actions_before_newton_solve(){
    ...
    Vector<double> u(1);
    TanhSolnOneDPoisson::get_exact_u(x, u);
...
{ 
    ...
    mesh_pt()->output_fct(exact_file,20*npts,TanhSolnOneDPoisson::get_exact_u); 
    exact_file.close();

    // Doc pointwise error and compute norm of error and of the solution
    double error,norm;
    ofstream error_file(("error" + to_string(label) + ".dat").c_str());
    mesh_pt()->compute_error(error_file,TanhSolnOneDPoisson::get_exact_u,
                            error,norm); 
    error_file.close();
...
int main(int argc, char* argv[]) {
    ...
    TanhSolnOneDPoisson::alpha = std::stoi(argv[1]);
    int n_elements = std::stoi(argv[2]);

    OneDPoissonProblem<QPoissonElement<1, 4>> problem(n_elements, TanhSolnOneDPoisson::source_function);
